...
![CI](https://github.com/alimtvnetwork/gitmap-v16/actions/workflows/ci.yml/badge.svg)
![golangci-lint](https://github.com/alimtvnetwork/gitmap-v16/actions/workflows/ci.yml/badge.svg?event=push)
![Vulncheck](https://github.com/alimtvnetwork/gitmap-v16/actions/workflows/vulncheck.yml/badge.svg)
![GitHub ](https://img.shields.io/github/v//alimtvnetwork/gitmap-v16?style=flat-square&label=version)
...
![Go Report Card](https://goreportcard.com/badge/github.com/alimtvnetwork/gitmap-v16/gitmap?style=flat-square)

_Scan, catalog, clone, and manage all your Git repositories from a single CLI._

<br>

<img src="docs/demo.gif" alt="Animated terminal demo of GitMap commands: clone, scan, history, , ssh, make-public/make-private, merge-left/right/both, and the interactive TUI" width="900">

<sub><i>30-second tour: clone · scan · history ·  · ssh · visibility · merge · interactive TUI</i></sub>
...
It manages Git repos (public and private), makes  smooth, and every repo we ship is released through GitMap itself.

---

## 🚀 Install

GitMap is a **Windows-first** project. The commands below install the latest  with sensible defaults — no prompts, no drive picker. Use the **Quick** block if you want to pick a custom install drive.
...
```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.ps1 | iex
```

```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.sh | bash
```

#### 🧭 Version matrix — `v4.1.0` install-script URLs

One-line installers and  assets for the pinned **v4.1.0** build across every supported platform. All URLs resolve to the exact  tag — no fallback, no discovery.

| Platform | Shell | Install-script URL |  binary asset |
| --- | --- | --- | --- |
| **Windows** (amd64) | PowerShell | [`-version-v4.1.0.ps1`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.ps1) | [`gitmap-v16.1.0-windows-amd64.zip`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/gitmap-v16.1.0-windows-amd64.zip) |
| **macOS** (arm64) | Bash | [`-version-v4.1.0.sh`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.sh) | [`gitmap-v16.1.0-darwin-arm64.tar.gz`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/gitmap-v16.1.0-darwin-arm64.tar.gz) |
| **macOS** (amd64) | Bash | [`-version-v4.1.0.sh`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.sh) | [`gitmap-v16.1.0-darwin-amd64.tar.gz`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/gitmap-v16.1.0-darwin-amd64.tar.gz) |
| **Linux** (amd64) | Bash | [`-version-v4.1.0.sh`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.sh) | [`gitmap-v16.1.0-linux-amd64.tar.gz`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/gitmap-v16.1.0-linux-amd64.tar.gz) |
| **Linux** (arm64) | Bash | [`-version-v4.1.0.sh`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/-version-v4.1.0.sh) | [`gitmap-v16.1.0-linux-arm64.tar.gz`](https://github.com/alimtvnetwork/gitmap-v16///v4.1.0/gitmap-v16.1.0-linux-arm64.tar.gz) |

> ** page:** [github.com/alimtvnetwork/gitmap-v16//tag/v4.1.0](https://github.com/alimtvnetwork/gitmap-v16//tag/v4.1.0) · **Asset naming contract:** `gitmap-<version>-<os>-<arch>.<ext>` (`.zip` on Windows, `.tar.gz` elsewhere) — verified by the installer pre-flight HEAD probe.
...
> **How install resolves a version:** every installer follows the generic contract in [`spec/07-generic-/09-generic-install-script-behavior.md`](spec/07-generic-/09-generic-install-script-behavior.md). In short — **strict tag mode** (`--version <tag>` / `-Version <tag>`) installs that exact  with **no fallback whatsoever** (no `latest`, no sibling probe, no main-branch HEAD; missing tag → exit 1). **Discovery mode** (no tag supplied) probes the next 20 `-v<N+i>` sibling repos in parallel, then falls back to `/latest`, and finally to the default branch HEAD as a last resort.
...
navigate, , and manage all of them as a single map — instead
...
- **One CLI** for the everyday Git chores that normally need 5
  ...
  merges,  packaging, interactive TUI, and shell completion.
...
- `history`, `stats`, and `` commands give you a per-repo
...
- `as` / `-alias` (`ra`) / `-alias-pull` (`rap`) —
  create labelled aliases of a  with concurrency-safe
  auto-stash/pop.
...
exit code — searchable, copy-paste-able, and synchronised with the
 metadata so the docs can never drift from the binary.
...
|------|-------------|---------|
| `-Version` | Pin a specific  | `-Version v2.51.0` |
...
| Flag | Description | Example |
|------|-------------|---------|
| `--version` | Pin a specific  | `--version v2.55.0` |
...
1. **Use `--allow-fallback`** — Automatically picks the newest patch in the same
   ...
   ```powershell
   irm https://github.com/alimtvnetwork/gitmap-v16///v3.38.0/-version-v3.38.0.ps1 | iex
   # Or with generic script:
   irm https://gitmap.dev/scripts/-version.ps1 | iex; Install-Gitmap -Version "v3.38.0" -AllowFallback
   ```
...
For reproducible installs, use the **per-version snapshot scripts** that are
 baked with the version at  time:

| Script | URL Pattern |
|--------|-------------|
| Pinned PowerShell | `https://github.com/alimtvnetwork/gitmap-v16///{version}/-version-{version}.ps1` |
| Pinned Bash | `https://github.com/alimtvnetwork/gitmap-v16///{version}/-version-{version}.sh` |
| Generic PowerShell | `https://gitmap.dev/scripts/-version.ps1` (requires `-Version` param) |
| Generic Bash | `https://gitmap.dev/scripts/-version.sh` (requires `--version` flag) |
...
> **Tip:** Use `gitmap list-versions` to see all available  versions before pinning.
...
The setup script installs the pre-commit hook (golangci-lint), verifies your Go toolchain, and  dependencies. See [CONTRIBUTING.md](CONTRIBUTING.md) for the full development workflow.
...
building**. Three  (v3.92.0, v3.113.0, v3.114.0) eliminated a
...
> **[`spec/07-generic-/09-generic-install-script-behavior.md`](spec/07-generic-/09-generic-install-script-behavior.md)**

It defines the two install modes in one place:

- **Strict tag mode** — explicit `--version <tag>` installs that exact
   with **no** fallback to `latest`, no `-v<N+i>` sibling probe,
...
- **Discovery mode** — no tag supplied → probe the next 20 `-v<N+i>`
  ...
  `/latest` → fall back to the default branch HEAD as a last
  resort.
...
These rules are stable across  and are enforced by
...
The CSV header is **stable across ** (locked by
...
- Build outputs (`dist/`, `target/`, `.next/`) that a 
...
> — defaults, allowed values, and the nested `` shape — is documented
...
The `gitmap ` command turns a clean working tree into a versioned,
tagged, multi-target GitHub  in one step — branch + tag + push +
binary build + checksum +  body + GitHub  page.
...
```bash
# End-to-end: register once,  from anywhere
cd ~/code/my-api
gitmap as my-api                  # one-time alias
cd ~                              # go anywhere
gitmap ra my-api v1.4.0 --pull    # pull --ff-only, , push, build, attach
```

> **Auto-stash safety:** dirty trees are stashed before `-alias`
...
Every successful  produces all of the following:

| Artifact | Where | Format |
|----------|-------|--------|
|  branch | local + `origin` | `/v3.51.0` |
| Annotated tag | local + `origin` | `v3.51.0` |
| Local manifest | `.gitmap//latest.json` | JSON (version, tag, branch) |
| GitHub  page | github.com/&lt;owner&gt;/&lt;repo&gt;/ | Title + body + assets |
|  body | GitHub  page | Markdown —  section + pinned-install snippet |
| Cross-compiled binaries (with `--bin`) | uploaded as  assets | `gitmap_v3.51.0_<os>_<arch>[.exe]` for 6 targets |
| Compressed archives (with `--compress`) | uploaded as  assets | `.zip` (Windows) / `.tar.gz` (Linux/macOS) |
| Checksums (with `--checksums`) | uploaded as  asset | `SHA256SUMS.txt` |
| Custom zip groups (with `--zip-group`) | uploaded as  assets | `.zip` bundles per group |

**Default build targets** (override with `--targets` or `.targets` in `config.json`):
...
```
  Creating  v3.51.0...
  ✓ Created branch /v3.51.0
  ✓ Created tag v3.51.0
  ✓ Pushed branch and tag to origin
  ✓  metadata written to .gitmap//latest.json
  ✓ Committed  metadata on /v3.51.0
  ✓ Marked v3.51.0 as latest 
  ✓ Using .md as  body
...
**Field meanings (-relevant only):**

| Field | Type | Default | Effect |
|-------|------|---------|--------|
| `.targets[]` | array of `{goos, goarch}` | built-in 6-target matrix | Cross-compile matrix used when `--bin` is set. `--targets` flag overrides this entirely. |
| `.targets[].goos` | string | — | Go `GOOS` value: `windows`, `linux`, `darwin`, `freebsd`, … |
| `.targets[].goarch` | string | — | Go `GOARCH` value: `amd64`, `arm64`, `386`, … |
| `.checksums` | bool | `false` | Always emit `SHA256SUMS.txt`. Equivalent to passing `--checksums` on every . |
| `.compress` | bool | `false` | Always wrap assets in `.zip`/`.tar.gz`. Equivalent to passing `--compress` on every . |
| `outputDir` | string | `./output` | Where non- CLI exports land (scan reports, etc). Not used by `` directly. |
| `excludeDirs` | array | `[]` | Folders to skip during scanning. Not used by `` directly. |
...
So `.compress: false` in config + `--compress` on the CLI → compression ON for that one run.

### CLI flag reference — `gitmap ` / `r`

```
gitmap  [version] [flags]
gitmap r       [version] [flags]
```

`version` is positional and optional. Forms accepted:

- `v3.51.0` —  exactly this tag
- *(omitted)* — auto-bump **minor** from the last  in `.gitmap//latest.json`, with a `[y/N]` prompt (skip with `-y`)
...
| `-N`, `--notes <text>` | string | git commit subject |  title / notes used as the GitHub  body header. |
| `--commit <sha>` | string | `HEAD` | Create the  from a specific commit instead of `HEAD`. Mutually exclusive with `--branch`. |
| `--branch <name>` | string | current branch | Create the  from the latest commit of `<name>`. Mutually exclusive with `--commit`. |
| `--assets <path>` | string | (none) | Single file or directory to attach as  assets (in addition to `--bin`, `-Z`, `--zip-group`). |
| `-b`, `--bin` | bool | `false` | Cross-compile Go binaries for every target in the matrix and attach them as  assets. |
| `--targets <list>` | string | from config / built-ins | Comma-separated `goos/goarch` pairs (e.g. `windows/amd64,linux/arm64`). Overrides `.targets` in config. |
| `--list-targets` | bool | `false` | Resolve the target matrix, print it, exit 0. No  is created. Useful for verifying config. |
| `--compress` | bool | `false` (or config) | Wrap each binary in a per-target archive: `.zip` for Windows, `.tar.gz` for Linux/macOS. |
| `--checksums` | bool | `false` (or config) | Emit `SHA256SUMS.txt` covering every uploaded asset and attach it to the . |
| `-Z <path>` | repeatable | (none) | Ad-hoc zip: include a single file or folder as a  asset. May be passed multiple times. |
| `--zip-group <name>` | repeatable | (none) | Attach a persistent named **zip group** (defined via `gitmap zg add`) as a  asset. May be passed multiple times. |
| `--bundle <name>` | string | (none) | Combine all `-Z` items into one archive named `<name>.zip` instead of one archive per item. |
| `--draft` | bool | `false` | Create the GitHub  as an unpublished draft. Branch/tag are still pushed. |
| `--dry-run` | bool | `false` | Print every step that would run (branch, tag, push, upload) without touching git or GitHub. |
| `--no-commit` | bool | `false` | Skip the post- auto-commit + push of `.gitmap//latest.json`. |
| `-y`, `--yes` | bool | `false` | Auto-confirm every prompt: bare- auto-bump, multi-repo scan, orphaned-metadata cleanup. |
...
| `--commit` **and** `--branch` | A  has exactly one source commit. |

→ Detailed help: [](gitmap/helptext/.md) · [-alias](gitmap/helptext/-alias.md) · [-self](gitmap/helptext/-self.md) · [-pending](gitmap/helptext/-pending.md) · [](gitmap/helptext/.md)

### Copy-paste workflows — scan, output, re-clone

End-to-end recipes for the data pipeline that feeds a : discover repos with `scan`, capture the result as CSV/JSON, and rebuild the same set on another machine with `clone`. Every block below is a single copy-paste — no placeholders to edit unless wrapped in `<…>`.
...
```bash
#  any aliased repo from anywhere — no `cd` required
gitmap as my-api                                   # one-time, run from inside the repo
gitmap -alias my-api v1.4.0
gitmap ra my-api v1.4.0 --pull                     # pull --ff-only, then 
gitmap -alias-pull my-api v1.4.0            # equivalent thin verb
gitmap rap my-api v1.4.0 --dry-run

gitmap -self --bump patch
gitmap tr 10 v1.$$ -s 5
```

> Dirty trees are auto-stashed before `-alias` runs and restored on
> exit. Pass `--no-stash` to abort instead, or `--dry-run` to preview.

→ [](gitmap/helptext/.md) · [-alias](gitmap/helptext/-alias.md) · [-alias-pull](gitmap/helptext/-alias-pull.md) · [-self](gitmap/helptext/-self.md) · [-branch](gitmap/helptext/-branch.md) · [temp-](gitmap/helptext/temp-.md)
...
| Command | Alias | Description |
|---------|-------|-------------|
| `zip-group` | `z` | Manage named file collections for  archives |

```bash
gitmap z create docs-bundle
gitmap z add docs-bundle ./README.md ./.md ./docs/
gitmap z show docs-bundle
gitmap  v3.0.0 --zip-group docs-bundle
```
...
| `make  BUMP=patch` | Lint, test, then  |
| `make -dry` | Preview  without executing |
